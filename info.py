...
syd_channel = environ.get('SYD_CHANNEL', '-1003442170595')
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1003387228939') #Channel / Group Id for force sub ( make sure bot is admin )
auth_grp = environ.get('AUTH_GROUP')
SYD_CHANNEL = int(syd_channel) if syd_channel and id_pattern.search(syd_channel) else None
AUTH_CHANNEL = [-1003442170595, -1003387228939, -1003325238947, -1003473523156, -1003469783090, -1003307071036, -1003469127200, -1003469783090, -1003496202907] #int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
